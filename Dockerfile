FROM nvidia/cuda:12.8.1-runtime-ubuntu22.04 as minimal
...
RUN apt update && \
    ...
    apt install -y python3 python3-pip python3-venv git wget libgl1-mesa-dev libglib2.0-0 libsm6 libxrender1 libxext6 libgoogle-perftools4 libtcmalloc-minimal4 libcusparse11 xdg-utils bc aria2 ffmpeg && \
...
ENV LD_PRELOAD=/usr/local/cuda-12.8/targets/x86_64-linux/lib/libcusparse.so.12
ENV PYTORCH_CUDA_ALLOC_CONF=garbage_collection_threshold:0.9,max_split_size_mb:512

ENTRYPOINT ["/app/entrypoint.sh", "--update-check", "--xformers", "--listen", "--enable-insecure-extension-access", "--api", "--port", "8080"]
